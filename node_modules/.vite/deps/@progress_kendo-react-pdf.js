...
var n2 = "export.pdf";
var h = "KendoReact PDF Generator";
var c = class {
  constructor(t4, o2, e7, i7, a9 = {}) {
    this.drawDOM = t4, this.exportPDF = o2, this.saveAs = e7, this.domElement = i7, this.options = a9, this.convertPageTemplateToHtml = (s6) => `<span>${r2.renderToStaticMarkup(
      p.createElement(this.options.pageTemplate, {
        pageNum: s6.pageNum,
        totalPages: s6.totalPages
      })
    )}</span>`;
  }
  savePDF(t4) {
    const o2 = this.drawDOM(this.domElement, this.getDrawOptions()).then((e7) => this.exportPDF(e7, this.getPDFOptions())).then((e7) => this.saveAs(e7, this.options.fileName || n2, this.getSaveOptions()));
    t4 && o2.then(t4, t4);
  }
  getDrawOptions() {
    return {
...
var e2 = Object.freeze({
  ...
  publishDate: 1755016193,
  version: "11.4.0",
  licensingDocsUrl: "https://www.telerik.com/kendo-react-ui/components/my-license/"
});

// node_modules/@progress/kendo-react-pdf/PDFExport.mjs
var s2 = class s3 extends t3.Component {
  constructor(r4) {
    super(r4), this.rootElForPDF = null, this.showLicenseWatermark = false, this.showLicenseWatermark = !d(e2, { component: "PDFExport" }), this.licenseMessage = w(e2);
  }
  /**
   * @hidden
   */
  render() {
    return t3.createElement(
      ...
      this.props.children,
      this.showLicenseWatermark && t3.createElement(P, { message: this.licenseMessage })
    );
  }
  /**
...
function f(r4, o2 = {}, e7) {
  new c(drawDOM, exportPDF, saveAs, r4, o2).savePDF(e7);
}
...
var c2 = (t4) => (s6) => l(s6, t4);
var f2 = (t4) => (e4[t4] || (e4[t4] = (s6) => String(s6.nodeName).toLowerCase() === t4.toLowerCase()), e4[t4]);
var a2 = (t4, s6, r4 = true) => {
  if (t4) {
    if (r4 && s6(t4))
      return t4;
...
var l2 = class {
  ...
  header(e7) {
    return this.headerWrap = this.headerWrap || a2(this.element, c2(o)), a2(this.headerWrap, c2(`${o}-${a3(e7)}`));
  }
  footer(e7) {
...
var h3 = "k-grid-footer";
var i3 = (e7, t4) => {
  const n6 = t4.length;
  for (let o2 = 0; o2 < n6; o2++)
    e7.appendChild(t4[o2].cloneNode(true));
};
var C = (e7) => {
...
var d2 = (e7) => {
  const t4 = e7.length, n6 = e7[0].cloneNode(true), o2 = n6.rows.length;
  if (t4 > 1)
    for (let l3 = 0; l3 < o2; l3++)
      for (let s6 = 1; s6 < t4; s6++)
        i3(n6.rows[l3], e7[s6].rows[l3].cells);
  return n6;
};
var f3 = (e7, t4) => {
  if (t4.length > 1 && e7.rows.length > 1)
    for (let n6 = 1; n6 < e7.rows.length; n6++) {
      const o2 = t4[0].rows[n6].cells.length, l3 = e7.rows[n6].cells[o2];
      String(l3.className).indexOf(a4) === -1 && (l3.className += ` ${a4}`);
    }
};
var m = (e7, t4, n6, o2) => {
  const l3 = document.createElement("table"), s6 = e7[0].cloneNode(true);
  for (let c5 = 1; c5 < e7.length; c5++)
    i3(s6, e7[c5].querySelectorAll("col"));
  const r4 = d2(t4), p2 = d2(n6);
  if (r4.className = g2, f3(r4, t4), l3.appendChild(s6), l3.appendChild(r4), l3.appendChild(p2), o2.length) {
    const c5 = d2(o2);
    c5.className = h3, l3.appendChild(c5);
  }
  return C(l3);
};
...
var b = (c5) => {
  const e7 = new l2(c5), o2 = e7.content();
  let t4;
  if (o2) {
    const n6 = [o2.querySelector("colgroup")], l3 = [e7.header().querySelector("thead")], s6 = [o2.querySelector("tbody")], r4 = e7.footer(), u3 = r4 ? [r4.querySelector("tfoot")] : [];
    t4 = m(n6, l3, s6, u3);
  } else
...
var F = u(m2());
function u(r4) {
  return (t4, o2 = {}, n6) => f4(r4, t4, o2, n6);
}
function f4(r4, t4, o2 = {}, n6) {
  let e7;
  d4();
  function d4() {
    r4(p2(), o2, a9);
  ...
  function p2() {
    ...
    const i7 = b(t4);
    return e7.appendChild(i7), document.body.appendChild(e7), i7;
  }
}
...
var i4 = class i5 extends r3.Component {
  constructor(t4) {
    super(t4), this.state = {
      show: false
    }, this.showLicenseWatermark = false, this.showLicenseWatermark = !d(e2, { component: "GridPDFExport" }), this.licenseMessage = w(e2), this.saveGridPDF = u(this.getSavePDF()), this.gridInnerWrapperRef = r3.createRef();
  }
  /**
   * @hidden
   */
  componentDidUpdate(t4, s6) {
    this.state.show && this.state.show !== s6.show && this.saveGridPDF(
  ...
  render() {
    return r3.createElement(r3.Fragment, null, this.state.show && (0, import_react_dom.createPortal)(
      ...
      document.body
    ), this.showLicenseWatermark && r3.createElement(P, { message: this.licenseMessage }));
  }
  ...
  save(t4, s6) {
    this.grid = this.getGrid(), this.columns = this.getCustomColumns(), this.data = t4, this.callback = () => {
      this.setState({ show: false }), s6 && s6();
  ...
  prepareRawGridForExport(t4) {
    const s6 = t4 && {
      ...
      pageSize: t4.length,
      skip: 0
    }, p2 = { style: Object.assign({}, this.grid.props.style, { width: "1000px" }) }, l3 = { scrollable: this.grid.props === "none" ? "none" : "scrollable" }, o2 = Object.assign({}, s6, p2, l3);
    if (this.columns && this.columns.length > 0) {
      const m5 = this.getGridNotColumnChildren(this.grid);
      return r3.cloneElement(this.grid, o2, this.columns.concat(m5));
    }
    return r3.cloneElement(this.grid, o2);
  }
  getGridNotColumnChildren(t4) {
    return r3.Children.toArray(t4.props.children).filter(
      (s6) => s6 && s6.type && s6.type.displayName !== "KendoReactGridColumn"
    );
  }
};
i4.propTypes = {
...
var a7 = class a8 extends s5.Component {
  constructor(t4) {
    super(t4), this.state = {
      show: false
    }, this.showLicenseWatermark = false, this.showLicenseWatermark = !d(e2, { component: "TreeListPDFExport" }), this.licenseMessage = w(e2), this.saveTreeListPDF = m3(this.getSavePDF()), this.treeListInnerWrapperRef = s5.createRef();
  }
  /**
   * @hidden
   */
   ...
   */
  render() {
    return s5.createElement(s5.Fragment, null, this.state.show && (0, import_react_dom2.createPortal)(
      s5.createElement(
        "div",
        {
          style: {
        ...
        s5.createElement("div", { ref: this.treeListInnerWrapperRef }, this.prepareRawTreeListForExport())
      ),
      document.body
    ), this.showLicenseWatermark && s5.createElement(P, { message: this.licenseMessage }));
  }
  /* eslint-disable max-len */
  /**
  ...
  prepareRawTreeListForExport() {
    ...
    return this.columns && this.columns.length > 0 ? s5.cloneElement(this.treeList, Object.assign({}, i7, { columns: this.columns })) : s5.cloneElement(this.treeList, i7);
  }
};
a7.propTypes = {
